    ...
    match node {
        ...
        StatementNode::Labeled { label, inner } => {
            ...
            if let StatementNode::For(r#for) = inner.as_ref() {
                ...
                // label has been set for this loop

                implicit::visit_for(ctx, r#for, Some(label.content()));
            } else {
        ...
        StatementNode::For(r#for) => implicit::visit_for(ctx, r#for, None),
        ...
        StatementNode::Continue { label, location } => {
            implicit::visit_continue(ctx, *label, location);
        }
        StatementNode::Break { label, location } => implicit::visit_break(ctx, *label, location),
