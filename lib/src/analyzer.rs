...
impl Analyzer {
    ...
    pub fn analyze(&self) -> Result<(), Vec<AnalysisError<'_>>> {
        ...
        if all_errors.is_empty() {
            ...
            Err(all_errors
                .into_iter()
                .map(ShallowEqAnalysisError::unwrap)
                .collect())
        }
    }
...
// deduplicate without comparing LabelBacktrace children, since near-identical
// errors (with different provenance trees) can be generated by, e.g., different
// build constraint permutations, but should be reported together as one
struct ShallowEqAnalysisError<'a>(AnalysisError<'a>);

impl<'a> ShallowEqAnalysisError<'a> {
    fn unwrap(self) -> AnalysisError<'a> {
        self.0
    }
}

impl PartialEq for ShallowEqAnalysisError<'_> {
    fn eq(&self, other: &Self) -> bool {
        self.0.file == other.0.file && self.0.kind.shallow_eq(&other.0.kind)
    }
}

impl Eq for ShallowEqAnalysisError<'_> {}

impl Hash for ShallowEqAnalysisError<'_> {
    fn hash<H: Hasher>(&self, state: &mut H) {
        self.0.file.hash(state);
        self.0.kind.shallow_hash(state);
    }
}
